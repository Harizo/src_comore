(function ()
{
    ...
    angular
        .module('app.comores.recommandation', ['ngCookies'])
        // .run(notification)        
        .config(config);
        var vs = {};
		var x =0;
   ...
   function config($stateProvider,  $translatePartialLoaderProvider, msNavigationServiceProvider)  {
        $stateProvider.state('app.comores_recommandation', {
            url      : '/recommandations',
            views    : {
                'content@app': {
                    templateUrl: 'app/main/comores/recommandation/recommandation.html',
		...
		msNavigationServiceProvider.saveItem('comores.recommandation', {
			title: 'Recommandations',
			icon  : 'icon-account-switch',
			weight: 10,
			state: 'app.comores_recommandation',
		/*	badge:vs,
            hidden: function()
            {
                 return hide_menu;
